...
(function() {
	...
	// 设置缩放
	com.setZoom = function(level) {
		// 限制缩放范围
		level = Math.max(com.zoom.min, Math.min(com.zoom.max, level));
		com.zoom.level = level;
	...
	// 初始化缩放
	com.initZoom = function() {
		var container = document.getElementById('chess-container');
		if (!container) return;

		// 按钮事件
		document.getElementById('zoom-in').onclick = function() {
			com.setZoom(com.zoom.level + com.zoom.step);
		};
		...
		container.onwheel = function(e) {
			e.preventDefault();
			var delta = e.deltaY > 0 ? -com.zoom.step : com.zoom.step;
			com.setZoom(com.zoom.level + delta);
		};
		...
		container.ontouchmove = function(e) {
			if (e.touches.length === 2) {
				...
				com.setZoom(initialScale * scale);
			}
		};

		// 双击放大
		container.ondblclick = function() {
			com.setZoom(com.zoom.level + 0.2);
		};
